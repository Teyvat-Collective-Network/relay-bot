  ...
  return (channel) => {
    const data = cleanObject({
      content: ((channel && reply)
        ? `**${reply.member?.displayName||reply.author.username}** from **${reply.guild.name}**: [original message](https://discord.com/channels/${channel.guild.id}/${channel.id}/${mirror(doc, channel).message})\n<:reply:918825589434097735>${content||''}`
...
=======
  ...
  return (channel) => {
    ...
    const reply = message.type === 'REPLY' && {
      author: replyMsg ? `**${replyMsg.member?.displayName||replyMsg.author.username}** from **${replyMsg.guild.name}**` : '**[Original Not Found]**',
