...
export function constructUser(message) {
  ...
  const override = await message.client.db.Nickname.findOne({ user_id: message.author.id });

  return {
    username: `${override?.nickname||message.member?.displayName||message.author?.username} from ${message.guild?.name}`,
