...
export async function filterInvites(client, text) {
  for (const match of text.match(regex.invite)) {
    try {
      const code = match.split("/").slice(-1)[0];
      const invite = await client.fetchInvite(code);
      if (process.env.GUILD_ALLOWLIST?.split(" ")?.includes(invite.guild.id)) continue;
      await client.tcn.fetchGuild(invite.guild.id);
      continue;
    } catch {}

    text = text.replaceAll(match, "<invite removed>");
  }
...
export async function constructUser(message) {
  ...
  return {
    username: await filterInvites(message.client, `${user?.nickname || message.member?.displayName || message.author?.username} from ${guild?.name || message.guild?.name}`),
    mdName: await filterInvites(message.client, `**${user?.nickname || message.member?.displayName || message.author?.username}** from **${guild?.name || message.guild?.name}**`),
...
export async function constructMessage(message, replyDoc) {
  const content = message.content && (await filterInvites(message.client, message.content))
    .split(regex.code)
...
export async function log(message, tag = '', global = {}) {
  ...
  data.content = `[${tag}](https://discord.com/users/${message.member?.id||message.author?.id||''}) ${global.name?`**[${global.name}]**`:''} ${message.content}`.slice(0,2000);
