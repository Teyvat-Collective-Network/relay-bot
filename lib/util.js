...
export async function constructUser(message) {
  ...
  return {
    username: `${user?.nickname || message.member?.displayName || message.author?.username} from ${guild?.name || message.guild?.name}`.replace(regex.invite, '<invite removed>'),
    mdName: `**${user?.nickname || message.member?.displayName || message.author?.username}** from **${guild?.name || message.guild?.name}**`.replace(regex.invite, '<invite removed>').replace(regex.title, m => m[0].split().map(p => `\\${p}`).join('')),
...
export async function getTCNData(interaction) {
  ...
  const [ guild, user ] = await Promise.all([
    ...
    interaction.client.tcn.fetchUser(userId).catch(() => {}),
  ]);
  return {
    guild,
    user,
    ...
    advisor: guild?.advisor === userId,
    voter: guild?.voter === userId,
  }
}
