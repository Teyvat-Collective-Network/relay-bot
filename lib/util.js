...
export function constructUser(message) {
  ...
  const nickname = await message.client.nickname(message.author);

  return {
    username: `${nickname||message.member?.displayName||message.author?.username} from ${message.guild?.name}`,
